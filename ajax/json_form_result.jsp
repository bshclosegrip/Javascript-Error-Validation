...
/* 오라클 데이터 베이스 연결
  ...
  ** TNS 오류나신 분들 보세요.
  C:\app\inst-2-31\product\18.0.0\dbhomeXE\network\admin\tnsnames.ora 파일 메모장으로 확인
  XE =
  (DESCRIPTION =
    (ADDRESS = (PROTOCOL = TCP)(HOST = DESKTOP-5QDP8R5)(PORT = 1521))
    (CONNECT_DATA =
      (SERVER = DEDICATED)
      (SERVICE_NAME = XE)
    )
...
*/
	...
	try{
		...
		String sql = "select first_name, email, phone_number, to_char(salary, '999,999') sal from employees " 
		           + whereSql ; // 4)
